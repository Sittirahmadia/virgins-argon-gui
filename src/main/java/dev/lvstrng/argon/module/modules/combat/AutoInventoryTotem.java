...
public final class AutoInventoryTotem extends Module implements TickListener {
	...
	private final BooleanSetting activateOnKey = new BooleanSetting(EncryptedString.of("Activate On Key"), false)
			.setDescription(EncryptedString.of("Only runs when you hold the selected keybind"));
	private final KeybindSetting activateKey = new KeybindSetting(EncryptedString.of("Activate Key"), GLFW.GLFW_KEY_C, false)
			.setDescription(EncryptedString.of("Hold this key to activate inventory toteming"));
	...
	public AutoInventoryTotem() {
		super(EncryptedString.of("Auto Inventory Totem"),
				...
				-1,
				Category.COMBAT);
		addSettings(mode, delay, hotbar, totemSlot, autoSwitch, forceTotem, autoOpen, stayOpenFor, activateOnKey, activateKey);
	}
	...
	public void onTick() {
		...
		if (activateOnKey.getValue()
				&& GLFW.glfwGetKey(mc.getWindow().getHandle(), activateKey.getKey()) != GLFW.GLFW_PRESS)
			return;
